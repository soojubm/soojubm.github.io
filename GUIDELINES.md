...
- 개수·단계 수 같은 수량("세 단계", "네 가지" 등)은 전달하지 않는다. 대상을 바로 제시하며, 이런 표현은 글줄만 늘릴 뿐 내용을 더하지 않는다.
- 바로 아래에 목록이나 표로 나오는 항목을 문장에서 미리 나열하지 않는다("A·B·C 상태를 갖습니다" 뒤에 A·B·C 목록이 이어지는 식).
...
- 콘텐츠나 레이아웃 구획을 위해 CSS `border`를 직접 사용하지 않는다. surface 또는 separator를 사용한다.
