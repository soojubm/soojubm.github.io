...
- 동작을 받는 컨트롤(닫기·액션 버튼 등)은 표시 여부를 별도 boolean prop으로 두지 않고, 소비자가 그 동작을 넘겼는지로 렌더를 정한다. 이벤트로 알리는 동작은 해당 이벤트의 구독 여부(`dismiss` 등), 설정 객체로 받는 동작은 그 prop의 유무(`primaryAction` 등)가 기준이다.
