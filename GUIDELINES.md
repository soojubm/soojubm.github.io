...
- 컴포넌트가 `:host { --token: 기본값; }`처럼 자기 토큰의 기본값을 스스로 선언해두면, 테마 블록이 조상에서 같은 이름을 재정의해도 적용되지 않는다(그 요소 자신에게 이미 선언된 값이 있으면 상속을 타지 않기 때문). 이런 컴포넌트를 테마에서 재정의하려면 별도 이름의 전용 토큰을 만들어 `var(전용-토큰, 기존-토큰)`처럼 소비하는 쪽에서 fallback으로 읽는다.
