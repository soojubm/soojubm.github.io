...
var router_1 = require("./javascripts/router");
var event_js_1 = require("./javascripts/event.js");
var input_1 = require("./javascripts/input");
var load_1 = require("./javascripts/load"); // loadSpinner
var validations_1 = require("./javascripts/validations");
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    sayHello.then(function (resolve) {
    ...
    window.addEventListener('scroll', function () {
        if (!document.querySelector('.post-head'))
            return;
...
var setGraph = function () {
    ...
    var graphItemBar;
    var graphValue;
    graphItems.forEach(function (element) {
...
function loginFormValidation() {
    ...
    // if(!login) return;
    alert();
    var loginEmail = document.querySelector('name=[email]');
    var loginPassword = document.querySelector('name=[password]');
    var helpers = document.querySelectorAll('.textfield-helper');
    // helpers.forEach(helper => helper.style.display = 'none');
    var loginData = { email: '', password: '' };
    document.addEventListener('input', function (event) {
        var target = event.target;
        var isEmail = target === loginEmail;
        var isPassword = target === loginPassword;
        console.log(target.validity);
        if (validations_1.validations.isNumber(loginEmail.value)) {
        ...
        // if(isEmpty) {}
        if (isEmail) {
            if (validations_1.validations.isRequired(loginEmail.value)) {
        ...
        if (isPassword) {
            ...
            if (validations_1.validations.isLength(loginPassword.value, 8)) {
...
exports.getTodayDate = function () {
    var today = new Date();
    var year = today.getFullYear(); // toString().padStart
